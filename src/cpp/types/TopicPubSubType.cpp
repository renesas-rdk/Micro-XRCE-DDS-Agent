...
TopicPubSubType::TopicPubSubType(bool with_key) {
    ...
    /* Do not auto-fill TypeInformation in discovery announcements.
     * FastDDS 3.5.x defaults auto_fill_type_information_=true; without a
     * registered TypeObject the hash is INVALID, which FastDDS 3.5 treats as
     * an explicit type mismatch and refuses SEDP endpoint matching.
     * FastDDS 3.6+ changed the mechanism: register_type_object_representation()
     * (empty by default) controls whether a TypeObject is registered; no
     * TypeObject → no TypeInformation in SEDP → legacy/untyped matching path.
     * Only call the 3.5.x API when building against those headers. */
#if defined(FASTDDS_VERSION_MAJOR) && defined(FASTDDS_VERSION_MINOR) && \
    FASTDDS_VERSION_MAJOR == 3 && FASTDDS_VERSION_MINOR < 6
